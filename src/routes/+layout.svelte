...
<div class="bg-base-100 justify-between fixed top-0 left-0 right-0">
    <div class="navbar max-w-3xl mx-auto justify-between">
        <!--left side of navbar-->
        <div>
            <a href="/" class="btn btn-ghost text-xl">MyOpinionNow</a>
            {#if session !== null}
                <a href="/{session.user.email}" class="btn btn-ghost">My Survey</a>
            {/if}
        </div>
        ...
        <div>
            ...
            {:else}
                <span class="text-black text-lg ml-2">{session.user.email}</span>
